    ...
    public class RunnerManager
    {
        ...
        public async Task<bool> StartWithRetriesAsync(string token, string ownerRepo, string githubUrl, int maxRetries = 5, int baseDelayMs = 200, CancellationToken cancellationToken = default)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                ...
                {
                    // Treat exceptions as transient failures and retry
        ...
        // High level orchestration: try to register (with retries) then start the runner containers.
        // Returns true only if both registration and starting containers succeed.
